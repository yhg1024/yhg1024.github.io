...
<body>
    ...
    formatNumber
     ...
     타입지정 : <fmt:formatNumber value="0.3" type="percent"></fmt:formatNumber>
     
     <h2>패턴을 지정해서 숫자를 활용</h2>
     0, # : 자릿수를 의미
     0 : 자리수를 지정 했는데 해당 자리에 수가 없으면 0을 출력
     # : 자리수를 지정했는데 해당 자리에 수가 없으면 출력하지 않음
     
     300
     00000 - 00300 - 자릿수를 맞추고 싶을때 (코드값을 출력)
     ##### - 300
     
     <!-- 출력 형식을 지정하는것을 매우 중요한 일이지만
     	보통은 쿼리를 실행하면서 포맷을 지정하여 dto에 담기 때문에
     	jstl 태그를 이용한 출력 형식 지정은 매우 드물게 사용될 수 있다. -->
     <c:set var="num1" value="12345.0111"></c:set><br>
     ...
     0,000,00.0 : <fmt:formatNumber value="${num1 }" pattern="0,000,00.0"></fmt:formatNumber><br>
